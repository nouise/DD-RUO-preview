...
from core.networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP, ResNet18ImageNet, ViT
...
class Config:
    #name=["tench","English springer","cassette player","chain saw","church","French horn","garbage truck","gas pump","golf ball","parachute"]
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if args.zca:
        ...
        save_and_print(args.log_path, f"device of dst_train:{dst_train[0][0].device}")
        save_and_print(args.log_path, f"device of dst_test:{dst_test[0][0].device}")

        for i in tqdm.tqdm(range(len(dst_train))):
            im, lab = dst_train[i]
            images.append(im)
            labels.append(lab)
        save_and_print(args.log_path, f"images: {len(images)}")
        images = torch.stack(images, dim=0)
        save_and_print(args.log_path, f"images shape: {images.shape}")
        save_and_print(args.log_path, f"images size in GB: {images.element_size() * images.nelement() / 1024 / 1024 / 1024:.2f} GB")
        save_and_print(args.log_path, f"Current device: {args.device}")
        save_and_print(args.log_path, f"GPU memory allocated: {torch.cuda.memory_allocated(args.device)/1024/1024:.2f} MB")
        save_and_print(args.log_path, f"GPU memory reserved: {torch.cuda.memory_reserved(args.device)/1024/1024:.2f} MB")
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=128, shuffle=False, num_workers=32)
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True):
    ...
    if model == 'MLP':
        ...
        net = ViT(image_size=im_size, patch_size=16, num_classes=num_classes,
                  dim=512, depth=10, heads=8, mlp_dim=512, dropout=0.01, emb_dropout=0.01)
